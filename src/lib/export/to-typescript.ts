 ...
 *
 * ⚠️  IMPORTANT: This module is for EXPORT ONLY, not runtime execution.
 *
 * Training Camp uses LiteLLM as the runtime LLM gateway for ALL agent execution.
 * This module exists solely to generate standalone TypeScript code that users
 * can take outside of Training Camp and run directly against the Anthropic API.
 *
 * DO NOT use this for runtime agent execution within Training Camp.
 * For runtime, see: src/api/llm.ts (LiteLLM client)
 *
 * The generated code:
 * - Uses the official @anthropic-ai/sdk package
 * - Is self-contained and can run independently
 * - Requires users to have their own ANTHROPIC_API_KEY
 */
...
export function exportToTypeScript(agent: AgentDefinition): string {
  ...
  // Use the agent's configured model directly
  const modelName = agent.parameters.model || 'claude-sonnet-4-20250514';
