...
describe('Tool Executor', () => {
  ...
  describe('executeToolCallsParallel()', () => {
    it('should execute multiple tool calls in parallel', async () => {
      ...
      const results = await executeToolCallsParallel(toolCalls, { createSpans: false });
    ...
    it('should handle empty array', async () => {
      const results = await executeToolCallsParallel([], { createSpans: false });
  ...
  describe('formatToolResultsForLLM()', () => {
    ...
    it('should format results for Claude format', () => {
      const formatted = formatToolResultsForLLM(mockResults, 'claude') as Array<{
        type: string;
        tool_use_id: string;
        content: string;
        is_error: boolean;
    ...
    it('should format results for OpenAI format', () => {
      const formatted = formatToolResultsForLLM(mockResults, 'openai') as Array<{
        role: string;
        tool_call_id: string;
        content: string;
      }>;
