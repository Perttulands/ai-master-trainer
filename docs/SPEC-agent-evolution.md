...
The following flow is fully functional:

```
User Score → Reward Analysis → Prompt Credit Assignment → Evolution Planning → Prompt Evolution → Artifact Generation
```

**Components**:
- `src/services/reward-analyzer.ts` - Parses scores/comments into structured feedback
- `src/services/credit-assignment.ts` - Assigns blame to prompt segments (prompt mode only)
- `src/services/evolution-planner.ts` - Generates targeted change plans
- `src/services/agent-evolver.ts` - Applies changes to agent definitions
- `src/services/agent-executor.ts` - Executes evolved agents

### Not Yet Functional ❌

**Trajectory Credit Assignment**:
- Code exists in `src/services/credit-assignment.ts` (trajectory mode)
- Requires execution spans from multi-step agent runs
- Blocked by: No flow/tool execution engine

**Multi-Step Agent Execution**:
- Agents define tools and flow steps
- But execution is single LLM call only
- Tools are never invoked, flow steps never followed

**Learning System Integration**:
- `extractLearning()` in pipeline extracts patterns
- But insights are inconsistently used in future planning
- History checking works but has limited data

### Key Files

| Purpose | File |
|---------|------|
| Evolution pipeline (full) | `src/services/evolution-pipeline.ts` |
| Simple evolver (used today) | `src/agents/agent-evolver.ts` |
| Lineage store (3 regen modes) | `src/store/lineages.ts` |

### Integration Points

The lineage store offers three regeneration methods:
1. `regenerateUnlocked()` - Direct content generation (no agent evolution)
2. `regenerateUnlockedWithAgents()` - Uses simple evolver
3. `regenerateWithFullPipeline()` - Uses complete evolution pipeline

Currently, the UI primarily uses method 2. Method 3 is available but underutilized.
