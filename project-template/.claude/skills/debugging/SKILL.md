...
- If **one workflow works and another fails**, assume **divergent code paths or split configuration sources** first. Write the two call graphs and compare inputs at the edge (network payload, DB writes, props), then search for hardcoded defaults.
...
| `Invalid model name passed in model=...` | Hardcoded model ID, wrong provider prefix, model alias not available for key, different code path uses different model selection |
